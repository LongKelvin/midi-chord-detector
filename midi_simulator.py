...
Uses rtmidi backend explicitly and handles Windows virtual port issues.
"""
...
class MIDISimulator:
    ...
    def setup(self) -> bool:
        """Create a virtual MIDI output port with rtmidi backend."""
        try:
            # Log available backends and ports for debugging
            logger.info(f"MIDO backend: {mido.backend}")
            logger.info(f"Available output ports: {mido.get_output_names()}")
            
            # Attempt to create a virtual MIDI port
            ...
            logger.info(f"Available output ports: {mido.get_output_names()}")
            logger.info("Try running with an existing MIDI port or installing a virtual MIDI tool like loopMIDI.")
            return False
